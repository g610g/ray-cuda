    ...
    def test_one_sided_v2(self):
        ...
        for nerr in range(1, num_errors + 1):
            for _ in range(max_iters):
                ...
                corrections_count = one_sided_v2(
                    local_read,
                    aux_corrections,
                    aux_kmer,
                    aux_kmer2,
                ...
                if corrections_count > 0:
                    for idx in range(seq_len):
                        if aux_corrections[idx] != 0:
                            print(
                                f"Changing position: {idx} to base: {aux_corrections[idx]}"
                            )
                            local_read[idx] = aux_corrections[idx]

            (max_votes, max_vote_idx) = cast_votes(
                local_read, votes, seq_len, kmer_length, bases, size, spectrum, aux_kmer
            )
            if max_votes == 0:
