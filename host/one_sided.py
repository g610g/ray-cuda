from host.host_helpers import identify_trusted_regions, select_mutations, transform_to_key, successor_v2, predeccessor_v2, give_kmer_multiplicity 
import ray
import numpy as np
...
def entry(reads, kmer_len, spectrum, offsets, num_errors, max_iters):
    ...
    for offset in offsets:
        ...
        #copy to local read
        original_read = reads[start: end].copy()
...
def one_sided(local_read, original_read, region_indices, selected_bases, kmer_len, seq_len, spectrum, solids, bases, max_corrections, distance ):
    ...
    regions_count = identify_trusted_regions(seq_len, spectrum, local_read, kmer_len, region_indices, solids , size)
    ...
    for region in range(regions_count):
        ...
        for target_pos in range(right_mer_idx + 1, seq_len):
            ...
            num_bases = select_mutations(spectrum, bases, ascii_kmer, kmer_len - 1, selected_bases)
            ...
            if num_bases == 1:
                ...
                for idx in range(0, num_bases):
                    if successor_v2(kmer_len, local_read, spectrum, selected_bases[idx], spos, distance, seq_len):
                ...
                if target_pos - last_position < kmer_len:
                    ...
                    if num_corrections > max_corrections:
                        for pos in range(last_position, target_pos + 1):
                            local_read[pos] = original_read[pos]
        ...
        if lkmer_idx > 0:
            ...
            for pos in range(lkmer_idx - 1, -1, -1):
                ...
                num_bases = select_mutations(spectrum, bases, ascii_kmer, 0, selected_bases)
                ...
                if num_bases == 1:
                    ...
                    for idx in range(0, num_bases):
                        if predeccessor_v2(kmer_len, original_read, spectrum, pos, selected_bases[idx], distance):
                            #might be redundant
                            aux_kmer = ascii_kmer.copy()
